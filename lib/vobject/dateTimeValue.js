...
module.exports = function(dateTimeString) {
  ...
  dateTimeValue.toICS = function() {
    if (dateTimeValue.tzid) {
      ...
      var tzid = tzlinks(dateTimeValue.tzid);

      if (tzid === 'Etc/GMT' || tzid === 'Etc/UTC') {
      ...
      try {
        return dateTimeValue.dateTime.tz(tzlinks(dateTimeValue.tzid)).format('YYYYMMDDTHHmmss');
      }
      catch (err) {
        console.error('Exception setting timezone', tzid, err);
      }
    }

    // Absolute Time: 20130813T173000Z
    return dateTimeValue.dateTime.utc().format('YYYYMMDDTHHmmss[Z]');
  };
